...
def count_TMs(HMMSEARCH_FILE_DIR: str, TM_INDICES: dict = {1: (0, 35), 2: (39, 69), 3: (73, 109), 4: (117, 144), 5: (161, 198), 6: (201, 237), 7: (239, 272)}):
    """Return: [{'name', 'accession','e-values','tm_count'}, ... ]    
    With given hmmout file and the TM indices of the profile GPCR, this function returns the amount of TMs in each accession.
    """

    GOOD_CANDIDATES=[]


    TM_START_INDICES=np.array([x[0] for x in TM_INDICES.values()])
    TM_END_INDICES=np.array([x[1] for x in TM_INDICES.values()])
