...
CROSS_SPECIES = ["human", "mouse"] #for other species go to other file (adding-other-species)
...
non_elim_path = os.path.join(general.HOME, f"synteny_analysis/UCSC_data/conservation/non_eliminative/{CROSS_SPECIES[0]}-{CROSS_SPECIES[1]}_syntenic_gene-pairs.tsv")
...
cant_eliminate = pd.DataFrame(columns=syn_columns)
...
for row in first_gene_pairs.itertuples():
    ...
    if (gene1 in second_species_genome) and (gene2 in second_species_genome):
        ...
        new_row = pd.DataFrame([list(row[1:]) + [np.nan]], columns=syn_columns)        
